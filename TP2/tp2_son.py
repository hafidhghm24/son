...
def spectro(signal, taille_fenetre, pas):
    # Nombre de fenêtres qu'on peut placer sur le signal
    nb_fenetre = (len(signal) - taille_fenetre) // pas + 1
    
    # Matrice résultat : demi-spectre (lignes) × nb fenêtres (colonnes)
    sp = np.zeros((taille_fenetre // 2, nb_fenetre))
    
    # Fenêtre de Hamming pour adoucir les bords (comme au TP1)
    fenetre_ham = np.hamming(taille_fenetre)
    
    for i in range(nb_fenetre):
        p = i * pas                                          # position de début
        segment = signal[p:p + taille_fenetre] * fenetre_ham # découper + fenêtrer
        spectre = np.fft.fft(segment)                        # FFT
        sp[:, i] = np.abs(spectre[:taille_fenetre // 2])     # demi-spectre → colonne i
...
def main():
    ...
    plt.figure(6)
    plt.plot(np.arange(len(s)) / fe, s)   # abscisses en secondes
    plt.title("Signal mélodique (arpège Do majeur)")
    plt.ylabel("Amplitude")               # c'est l'amplitude, PAS la fréquence
    plt.xlabel("Temps (s)")
    plt.show()
    ...
    taille_fenetre = 1024   # w = 1024 (demandé par le TP)
    pas = 512               # p = 512 (recouvrement de moitié)
    ...
    plt.figure(7)
    plt.imshow(
        20 * np.log10(spectroM + 1e-6),               # en dB pour mieux voir
